...
- **分支 4 是吸收态, 不只是"丢一次恢复"**: 一个身份一旦以无 pid 形态落库 (例如某次 auto-bind 走了 tty 分支而非 pid 分支), 之后**每一次**座位重建都会命中分支 4, 直到有人让它重新带 pid 注册 —— 而"需要人工介入"正是本变更要消灭的病.  → `holder_liveness_unknown` 日志写明后果, 使运维看得出这是会复发的状态而非一次性 skip.
- **对某些 runtime, 解除条件不会自己到来**: 解除分支 4 需要该身份带正数 pid 注册一次, 而 kimi-code 与 opencode 结构上不产生 pid (`inferRuntimeAgentKind` 对 kimi 直接 return undefined; opencode 的 pid 绑定分支被 `inferredAgent === 'codex'` 挡住).  2026-08-14 生产库佐证: 曾有正数 pid 的行 claude-code 128 / codex 24 / opencode 1 / **kimi-code 0**.  所以对一个稳定跑在 kimi 上的身份, 分支 4 实践上是永久的 (逻辑上仍可解除 —— 该 `(team, name)` 只要以 codex 或 claude-code 形态注册过一次就会带上 pid).  **判据不因此放宽**: "证明不了它死了"就是不能动它, 放宽的直接后果就是静默接管一个活着的持有者.  → 日志措辞必须同时点明解除条件与"哪些 runtime 不会自己满足它", 否则运维分不清"等一会儿就好"和"这里需要人介入".  日后要收回这部分代价, 抓手有两个: tty 绑定的 codex 行可用 `runtime_tty` / `tmux_pane_id`; kimi 与 opencode 的行带着 `base_url` + `session_id`, 可探询那个 server 的 session 是否仍在 —— 后者才是对症的那个, 因为它是这两类 runtime 唯一可得的 positive liveness evidence.  两者都引入网络探测与超时语义, 属独立变更.
