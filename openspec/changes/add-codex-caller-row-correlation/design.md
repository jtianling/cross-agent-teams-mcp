...
## 生产实测: key 进入预注册行对候选阶段**零作用** (2026-07-31)

两种组合各测一次, 结果**逐字相同**, 所以"等 key 真的进了行也许就好了"这条希望**彻底关闭**:

| 组合 | 结果 |
| --- | --- |
| 一条带 key (`%82`) + 一条不带 (`%83`) | `candidates=2 pending=2` → fail-closed |
| **两条都带 key** (`%82`/`%83` 各自的 key) | `candidates=2 pending=2` → fail-closed, 与上行同形 |

机制上本就如此: `candidate_count` 是在**逐行评估之后**统计的, 而逐行的 key 判定只**剔除外来行**
(`classifyRowClaim`).  两条行的 key 都**没有 holder** (从未被消费, 因此从未附到任何 agents 行)
→ `findByIdentityKey` 空 → 判定不外来 → 两行都留在候选集.

**由此解开一个此前的困惑**: 更早那批日志是 `candidates=0` 而不是 2 —— 因为当时两个 caller
**自己持有 key**, `identity_key_contradiction` 把行剔掉了.  也就是说**"caller 持 key"会意外地
把候选数压下去**: 一个缺陷偶然遮住了另一个缺陷, 这是当时归因只看到 key 矛盾的原因.
...
## 真身复核结果 (2026-07-31, 私有实验室, 工作树 dist)

**N3 (红证) / N1 / N2 —— 红、绿、绿.**  前置三步齐 (build → 重启 lab daemon → **读 argv 确认**
加载的是工作树 `dist/cli.js`).

- **N3 红**: `reason=candidate_count candidates=2 pending=2 panes=%106,%105`, 两条行都在,
  未被消费 → **夹具确实构造出了两个合格候选**, 所以 N1/N2 的绿才有意义.
  (新代码里 N1/N2 的红已产不出来, 所以红证只能来自同一份夹具**去掉 nonce**的那一臂.)
- **N1 绿**: 两条行**各被各自 caller 消费**, 两个 agents 行都有 pane + 新载体 pid + 原 key,
  全程无 `candidate_count`.  关键的一行是 `auto-bind targeted … rows=1 pending_total=2` ——
  **两条行同时待定时仍然定向成功**, 不是候选恰好唯一才过的.
- **N2 绿**: 无关会话的行 (key 不同, 防假绿) **原样保留**, 恢复方正常绑上.

### M9 唯一不可控的那一环: 真身**原样带回了 nonce, 三次全中**

传了、逐字相同、**模型自己读话术做的, 无任何提示**; daemon 侧三次 `outcome=resolved` 独立印证.

**而且它给出了一条可复用的规则**: 今天早些时候踩过的"模型把 `$CODEX_THREAD_ID` 当字面量传"
那个坑**没有复现** —— 差别在于**话术把值直接写在文本里, 不需要模型去解引用**.

> **要模型"取"一个值容易出事; 直接把值给它不会.**

所以话术**不需要写得更强硬**.  但 `outcome=unknown` 那条日志要保留: 万一将来模型开始"整理"
nonce, 失败是**静默回落**, 那条日志是唯一看得见的信号.

### 已验证的边界 (aoe 主动披露, 必须照收)

**验到的是定向逻辑本身**; **没验**的是"aoe 的 Shift+C 能否把双 pane 送进这个状态".

N1/N2 的"重启"并非全部由 Shift+C 驱动: 第一次恢复是真 Shift+C (走完
`scheduled → detected → resume → delivered`), 双 pane 那次与 N2 是用 `respawn-pane -k` 换载体
+ CLI 重新登记行构造的.  挡住完整路径的是两条实测事实: 该实验室会话里 aoe **没给 extra pane
铸出 slot key** (无 key 即无身份, 右 pane 不会被 poke); 以及 daemon **正确地**拒绝为"持有者
pid 仍存活"的 pane 安排恢复.

**后续 (aoe 查完, 两个事实非推断): 不是缺陷, 是时序.**  那个实验室实例
`cross_agent_team = True` (排除"会话没开 xats"), 而 `slot 1` 的**第一次 adopt 比 Shift+C 晚
14 秒** —— 按下那一刻 `agent_slot` 里只有 slot 0, 扇出只拿到一行, **没有 slot 1 可铸 key**.
根因与昨天量过的是同一条: **extra codex pane 要先被 rollout 认领才会进 slot**, 而真身的认领
延迟是**几十秒到几分钟**量级 (昨天 44s / 77s; 这次 slot0→slot1 相隔 2 分 37 秒).

**本 change 不得据此宣称"一次 Shift+C 两个 pane 都能自动恢复"** —— 只能宣称"当两个 pane 都
进入待恢复状态时, 定向能让它们各自认领自己那一行".  **而"都进入待恢复状态"本身要求两个 pane
都已被 aoe 认领进 slot**, 那在真身上是按下 Shift+C 之后几十秒到几分钟的事.  所以那句话成立
与否**还取决于用户按 Shift+C 的时机**, 不只取决于 M9 —— 这不是 M9 的问题, 但它是那句话的
必要条件, 写在同一处免得日后被拆开引用.

### 审批对话框

三次恢复注册**三次都停在审批对话框等人**.  所以"自动恢复"的准确口径仍是
"**自动引导到一次需要用户确认的注册**", 在没有预授权的机器上不构成全自动闭环.
