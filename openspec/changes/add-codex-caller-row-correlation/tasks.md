...
- [ ] 1c.2 改按 **M9** (恢复 poke 携带 nonce, 注册回带) 实现: daemon 对 nonce↔pane 的对应是确定的, 不是排除法, 不需要 aoe 配合; 漏带即回落现状 (照做严格更好, 不照做不比现在差)
...
- [ ] 2.1 恢复 poke 生成一次性 nonce 并与目标 pane 一起记账; 话术要求 codex 注册时回带; `register_agent` 接受可选 nonce, 命中即确定选中该 pane 的行; 未命中/未带一律回落现状, **决不猜**
