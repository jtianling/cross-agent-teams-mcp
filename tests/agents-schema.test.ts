...
describe('agents schema', () => {
  ...
  it('creates agents table with required columns and name is NOT NULL', () => {
    ...
    expect(names).toEqual([
      'agent_id','agent_type','agent_type_name','channel_session_id','claude_ui_pid','delivery_kind','delivery_payload','device','identity_key','last_processed_event_id','last_seen_at','model','name','prev_tmux_pane_id','register_generation','registered_at','remote_addr','role','runtime_bound_at','runtime_tty','runtime_ui_pid','runtime_verification_mode','team','tmux_pane_id'
    ])
